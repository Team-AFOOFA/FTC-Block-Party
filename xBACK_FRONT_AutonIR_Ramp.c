...
task main()
{
    ...
    motor[LeftDrive] = -30;
		...
		while((LeftIRVal != 6)&& (nMotorEncoder[RightDrive] > -5400)) //move back until robot is perpendicular to IR beacon
		{
			motor[LeftDrive] = -50; //both used to be -70, but were reduced
			motor[RightDrive] = -50; //to decrease inertia while avoiding alliance robot
			wait1Msec(2);
		...
		motor[RightDrive] = 0;
		wait1Msec(200); // stabilize

		nxtDisplayCenteredTextLine(3, "%d", nMotorEncoder[RightDrive]);//display current encoder values for drive train motors on screen

		if ( abs(nMotorEncoder[RightDrive]) >=5440) {
			// IR is placed in 4'th basket

		} else
		{
			// If we did not find IR Beacon in 1 or 2 baskets ; Adjust for Beacon offset to Basket
    ...
    */
		...
		Move(70, 70, 4000, 4000);
		Move(70, -70, 2100, 2100);
